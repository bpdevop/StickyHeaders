...
open class StickyAdapter : RecyclerView.Adapter<StickyAdapter.ViewHolder>() {
    ...
    private lateinit var sectionIndicesByAdapterPosition: IntArray
    ...
    open class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        open fun getItemViewBaseType(): Int = unmaskBaseViewType(itemViewType)
        open fun getItemViewUserType(): Int = unmaskUserViewType(itemViewType)

        open fun isHeader(): Boolean = false
        open fun isGhostHeader(): Boolean = false
        open fun isFooter(): Boolean = false

        open fun getSection(): Int = section

        internal fun setSection(section: Int) {
            this.section = section
        }

        open fun getNumberOfItemsInSection(): Int = numberOfItemsInSection

        open fun setNumberOfItemsInSection(numberOfItemsInSection: Int) {
            this.numberOfItemsInSection = numberOfItemsInSection
        }
    }

    open class ItemViewHolder(itemView: View) : ViewHolder(itemView) {
        private var positionInSection = 0

        fun getPositionInSection(): Int = positionInSection

        internal fun setPositionInSection(positionInSection: Int) {
     ...
     */
    open fun getSectionForAdapterPosition(adapterPosition: Int): Int {
        if (sections == null) {
            buildSectionIndex()
        }
        if (itemCount == 0) {
     ...
     */
    open fun getPositionOfItemInSection(sectionIndex: Int, adapterPosition: Int): Int {
        if (sections == null) buildSectionIndex()
        if (sectionIndex < 0) throw IndexOutOfBoundsException("sectionIndex $sectionIndex < 0")

        if (sectionIndex >= sections!!.size) throw IndexOutOfBoundsException("sectionIndex $sectionIndex >= sections.size (${sections!!.size})")

        val section: Section = sections!![sectionIndex]
        var localPosition: Int = adapterPosition - section.adapterPosition
        if (localPosition > section.length) throw IndexOutOfBoundsException("adapterPosition: $adapterPosition is beyond sectionIndex: $sectionIndex length: ${section.length}")

        if (section.hasHeader) localPosition -= 2 // adjust for header and ghostHeader
    ...
    private fun getAdapterPosition(sectionIndex: Int, offsetIntoSection: Int): Int {
        ...
        val section: Section = sections!![sectionIndex]
        val adapterPosition: Int = section.adapterPosition
     ...
     */
    open fun getAdapterPositionForSectionItem(sectionIndex: Int, offsetIntoSection: Int): Int =
        if (doesSectionHaveHeader(sectionIndex)) getAdapterPosition(sectionIndex, offsetIntoSection) + 2 // header is at position 0, ghostHeader at position 1
        else getAdapterPosition(sectionIndex, offsetIntoSection)
     ...
     */
    open fun getAdapterPositionForSectionFooter(sectionIndex: Int): Int = if (doesSectionHaveFooter(sectionIndex)) {
        val section: Section = sections!![sectionIndex]
        val adapterPosition: Int = section.adapterPosition
     ...
     */
    open fun setSectionIsCollapsed(sectionIndex: Int, collapsed: Boolean) {
        val notify = isSectionCollapsed(sectionIndex) != collapsed
        collapsedSections[sectionIndex] = collapsed
        if (notify) {
            if (sections == null) buildSectionIndex()
            val section: Section = sections!![sectionIndex]
            val number: Int = section.numberOfItems
            if (collapsed) notifySectionItemRangeRemoved(sectionIndex, 0, number, false) else notifySectionItemRangeInserted(sectionIndex, 0, number, false)
        }
    }
     ...
     */
    open fun isSectionCollapsed(sectionIndex: Int): Boolean = if (collapsedSections.containsKey(sectionIndex)) collapsedSections[sectionIndex]!! else false
     ...
     */
    open fun clearSelection(notify: Boolean) {
            ...
            for (sectionIndex in selectionState!!.keys) {
                ...
                state?.let {
                    if (it.section) {
                        ...
                        val s = it.items.size()
                        while (i < s) {
                            if (it.items.valueAt(i)) {
                                notifySectionItemChanged(sectionIndex, it.items.keyAt(i))
                            }
                            i++
                        }
                        if (it.footer) notifySectionFooterChanged(sectionIndex)
                    }
                }
            }
        }
     ...
     */
    open fun isSelectionEmpty(): Boolean {
        for (sectionIndex in selectionStateBySection.keys) {
            ...
            if (state!!.section) return false
                ...
                while (i < s) {
                    if (state.items.valueAt(i)) return false
                    i++
                }
    ...
    open fun getSelectedItemCount(): Int {
        ...
        for (sectionIndex in selectionStateBySection.keys) {
            ...
            if (state!!.section) {
                ...
                if (doesSectionHaveFooter(sectionIndex)) count++
            } else {
                var i = 0
                val s = state.items.size()
                while (i < s) {
     ...
     */
    open fun traverseSelection(visitor: SelectionVisitor) {
        // walk the section indices backwards
        val sectionIndices: List<Int> = ArrayList(selectionStateBySection.keys)
        ...
        for (sectionIndex in sectionIndices) {
            ...
            if (state.section) visitor.onVisitSelectedSection(sectionIndex)
            else {
                if (state.footer) visitor.onVisitSelectedFooter(sectionIndex)
                // walk items backwards
                for (i in state.items.size() - 1 downTo 0) if (state.items.valueAt(i)) visitor.onVisitSelectedSectionItem(sectionIndex, state.items.keyAt(i))
            }
        }
    }
     ...
     */
    open fun setSectionSelected(sectionIndex: Int, selected: Boolean) {
     ...
     * @param sectionIndex index of section
     */
    open fun toggleSectionSelected(sectionIndex: Int) {
     ...
     */
    open fun isSectionSelected(sectionIndex: Int): Boolean = getSectionSelectionState(sectionIndex).section
     ...
     * @param selected     selection state
     */
    open fun setSectionItemSelected(sectionIndex: Int, itemIndex: Int, selected: Boolean) {
        val state = getSectionSelectionState(sectionIndex)
     ...
     */
    open fun toggleSectionItemSelected(sectionIndex: Int, itemIndex: Int) = setSectionItemSelected(sectionIndex, itemIndex, !isSectionItemSelected(sectionIndex, itemIndex))
     ...
     */
    open fun isSectionItemSelected(sectionIndex: Int, itemIndex: Int): Boolean {
        val state = getSectionSelectionState(sectionIndex)
     ...
     * @param selected     selection state
     */
    open fun setSectionFooterSelected(sectionIndex: Int, selected: Boolean) {
        val state = getSectionSelectionState(sectionIndex)
        if (state.section) return
     ...
     * @param sectionIndex index of section
     */
    open fun toggleSectionFooterSelection(sectionIndex: Int) = setSectionFooterSelected(sectionIndex, !isSectionFooterSelected(sectionIndex))
     ...
     */
    open fun isSectionFooterSelected(sectionIndex: Int): Boolean {
        val state = getSectionSelectionState(sectionIndex)
     ...
     */
    open fun notifySectionDataSetChanged(sectionIndex: Int) {
        if (sections == null) {
            buildSectionIndex()
     ...
     */
    open fun notifySectionItemRangeInserted(sectionIndex: Int, fromPosition: Int, number: Int) {
    ...
    private fun notifySectionItemRangeInserted(sectionIndex: Int, fromPosition: Int, number: Int, updateSelectionState: Boolean) {
        if (sections == null) {
            ...
            if (fromPosition > section.numberOfItems) throw IndexOutOfBoundsException("itemIndex adapterPosition: $fromPosition exceeds sectionIndex numberOfItems: ${section.numberOfItems}")
     ...
     */
    open fun notifySectionItemRangeRemoved(sectionIndex: Int, fromPosition: Int, number: Int) {
    ...
    private fun notifySectionItemRangeRemoved(sectionIndex: Int, fromPosition: Int, number: Int, updateSelectionState: Boolean) {
        if (sections == null) {
            ...
            if (fromPosition + number > section.numberOfItems) throw IndexOutOfBoundsException("itemIndex adapterPosition: ${fromPosition + number} exceeds sectionIndex numberOfItems: ${section.numberOfItems}")
     ...
     */
    open fun notifySectionItemInserted(sectionIndex: Int, itemIndex: Int) {
        if (sections == null) {
            buildSectionIndex()
            notifyAllSectionsDataSetChanged()
        } else {
     ...
     */
    open fun notifySectionItemRemoved(sectionIndex: Int, itemIndex: Int) {
        if (sections == null) {
            buildSectionIndex()
            notifyAllSectionsDataSetChanged()
        } else {
     ...
     */
    open fun notifySectionInserted(sectionIndex: Int) {
        if (sections == null) {
            buildSectionIndex()
            notifyAllSectionsDataSetChanged()
        } else {
     ...
     */
    open fun notifySectionRemoved(sectionIndex: Int) {
        if (sections == null) {
            buildSectionIndex()
            notifyAllSectionsDataSetChanged()
        } else {
     ...
     */
    open fun notifySectionFooterInserted(sectionIndex: Int) {
        if (sections == null) {
            buildSectionIndex()
            notifyAllSectionsDataSetChanged()
        } else {
     ...
     * @param sectionIndex position of the section
     */
    open fun notifySectionFooterRemoved(sectionIndex: Int) {
        if (sections == null) {
            buildSectionIndex()
            notifyAllSectionsDataSetChanged()
        } else {
     ...
     * @param sectionIndex position of the section
     */
    open fun notifySectionFooterChanged(sectionIndex: Int) {
        if (sections == null) {
            buildSectionIndex()
            notifyAllSectionsDataSetChanged()
        } else {
    ...
    private fun buildSectionIndex() {
        ...
        val ns = getNumberOfSections()
        while (s < ns) {
            val section: Section = sections!![s]
            for (p in 0 until section.length) sectionIndicesByAdapterPosition[i + p] = s
            i += section.length
            s++
        }
    }
    ...
    private fun updateSectionItemRangeSelectionState(sectionIndex: Int, fromPosition: Int, delta: Int) {
        ...
        while (i < n) {
            ...
            var newPos = pos
            if (pos >= fromPosition) newPos += delta

            if (itemState[pos]) sectionSelectionState.items.put(newPos, true)
            i++
        }
    }
    ...
    private fun updateCollapseAndSelectionStateForSectionChange(sectionIndex: Int, delta: Int) {
        ...
        for (i in collapseState.keys) {
            // erasure
            if (delta < 0 && i == sectionIndex) continue

            var j = i
            if (j >= sectionIndex) j += delta
    ...
    override fun getItemViewType(adapterPosition: Int): Int {
        ...
        val baseType: Int = getItemViewBaseType(section, localPosition)
        var userType = 0
    ...
    open fun getItemViewBaseType(section: Section, localPosition: Int): Int = if (section.hasHeader && section.hasFooter) {
        ...
        if (localPosition == section.length - 1) TYPE_FOOTER else TYPE_ITEM
    } else TYPE_ITEM // this sections has no header or footer

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val baseViewType: Int = unmaskBaseViewType(viewType)
        val userViewType: Int = unmaskUserViewType(viewType)
    ...
    override fun onBindViewHolder(holder: ViewHolder, adapterPosition: Int) {
        ...
        val baseType: Int = unmaskBaseViewType(holder.itemViewType)
        val userType: Int = unmaskUserViewType(holder.itemViewType)
        when (baseType) {
     ...
     */
    open fun tagViewHolderItemView(holder: ViewHolder, section: Int, adapterPosition: Int) = holder.itemView.setTag(R.id.adapter_tag_key_viewholder, holder)
}